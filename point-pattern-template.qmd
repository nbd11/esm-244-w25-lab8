...
Controls variance of spatial distrubtion for the density plot
...
```{r}
gfunction_data<-as.data.frame(gfunction_out)|>
  pivot_longer(cols=c(2:5))


ggplot(data=gfunction_data)+
  geom_line(aes(x=r,y=value,color=name))+
  xlim(0,4500)+
  theme_bw()
```
...
### Make a sequence of distances over which you'll calculate L(r)
rL_vec <- seq(0, 80000, by = 5000) 

lfunction_out <- envelope(voles_full, fun = Lest, r = rL_vec, 
                          nsim = 10) 


lfunction_data<-as.data.frame(lfunction_out)|>
  pivot_longer(cols=obs:hi,names_to="model",values_to="l_val")

plot(lfunction_out)

ggplot(lfunction_data)+
  geom_line(aes(x=r,y=l_val,color=model))+
  theme_bw()
```
